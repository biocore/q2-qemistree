...
def match_label(collated_fingerprints: pd.DataFrame,
    ...
    # TODO:  use biom instead of pandas for feature table
    ...
    relabeled_table = biom.table.Table(
    	filtered_data, allfps, features.ids('sample'), [{'label': str(val)} for val in list_md5])
    bin_f = lambda id_, x: x['label']
	collapsed_table = relabeled_table.collapse(bin_f, norm=True, 
    	min_group_size=1,axis='observation').sort(axis='observation')
